...
(function (root) {
     ...
     */
    var element = {
        byId: function byId(tagId) {
            return document.getElementById(tagId.replace("#", ""));
        },
        byClass: function byClass(tagClass, par) {
            var result = [],
                d = par || document;
        ...
        byTag: function byTag(tagName, par) {
            return (par || document).getElementsByTagName(tagName);
        },
        byName: function byName(attrName, par) {
    ...
    var addStyle = function addStyle(str) {
    ...
    colorTake.prototype = {
        ...
        joinStr: function joinStr() {
            ...
            var createColor = function createColor(baseColor) {
                var colorLiStr = '';
                for (var j = 0; j < 6; j++) {
                    for (var k = 0; k < 6; k++) {
        ...
        showColorPanle: function showColorPanle(showColorPanleEle, getColorPanleColor, callback) {
            ...
            eventUnit.addEvent(showColorPanleEle, "click", function () {
                ...
                var colorPanleTempStyle = "position:absolute;left:" + showColorPanleEle.offsetLeft + "px;top:" + (50 + showColorPanleEle.offsetTop) + "px;display:block",
                ...
                var colorRead = function colorRead(eventType, e) {
                    return function (e) {
